...
class TestMideaACDevice:
    ...
    def test_build_query_fallback_probes_the_subprotocol_family(self) -> None:
        """A silent primary family must be re-probed with the BB queries.

        223J6397 answers the appliance query and the subprotocol queries and
        none of the B5/0x41 status queries, so the checked probe blacklists the
        whole primary family and the device never becomes available. The
        fallback family is what recovers it.
        """
